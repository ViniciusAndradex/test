...
void changeIdWhiteList({Function()? callback}) async {
  var newIdWhiteList = (await bind.mainGetOption(key: 'id_whitelist')).split(',');
  var newIdWhiteListField = newIdWhiteList.join('\n');
  ...
  gFFI.dialogManager.show((setState, close, context) {
    return CustomAlertDialog(
      ...
      actions: [
        dialogButton("Cancel", onPressed: close, isOutline: true),
        dialogButton("Clear", onPressed: () async {
          await bind.mainSetOption(key: 'id_whitelist', value: '');
          callback?.call();
          close();
        }, isOutline: true),
        dialogButton(
          ...
          onPressed: () async {
            ...
            if (newIdWhiteListField.isEmpty) {
              ...
              final ips = newIdWhiteListField.trim().split(RegExp(r"[\s,;\n]+"));
              // test ip
//               final ipMatch = RegExp(
//                   r"^(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]?|0)\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]?|0)\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]?|0)\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]?|0)(\/([1-9]|[1-2][0-9]|3[0-2])){0,1}$");
//               final ipv6Match = RegExp(
...
//                 }
//               }
              newIdWhiteList = ips.join(',');
            }
            await bind.mainSetOption(key: 'id_whitelist', value: newIdWhiteList);
            callback?.call();
            close();
          },
        ),
